...
### Domain-aware architecture generation

```bash
python run_architecture_skill.py --text-file examples/request.txt --domain financial_services --allow-parallel
```

### Claude-native rendering path

```bash
python run_architecture_skill.py --text-file examples/request.txt --render --render-mode claude-native
```

### Prose/article extraction mode
...
Use `pattern` on flows to distinguish sync API, async event, batch ETL, streaming, webhook, pub/sub, gRPC, file transfer, and replication connectors.

### Annotation layer
Use `annotations` to add highlights, risk flags, callouts, decision points, and investment markers.
